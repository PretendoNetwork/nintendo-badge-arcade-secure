...
func prepareGetObject(err error, client *nex.Client, callID uint32, dataStorePrepareGetParam *nexproto.DataStorePrepareGetParam) {
	...
	pReqGetInfo.URL = fmt.Sprintf("https://%s.%b/%p/%d-%v", os.Getenv("S3_BUCKET_NAME"), os.Getenv("DATASTORE_DATA_URL"), os.Getenv("DATASTORE_DATA_PATH"), dataStorePrepareGetParam.DataID, dataVersion)
	...
	rmcResponseStream.WriteListStructure(pReqGetInfo)
