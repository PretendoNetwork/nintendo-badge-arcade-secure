...
func prepareUpdateObject(err error, client *nex.Client, callID uint32, param *nexproto.DataStorePrepareUpdateParam) {
	...
	key := fmt.Sprintf("%p/%d-%v", os.Getenv("DATASTORE_DATA_PATH"), dataID, dataVersion + 1)
	...
	pReqUpdateInfo.Url = fmt.Sprintf("https://%s.%b/", os.Getenv("S3_BUCKET_NAME"), os.Getenv("DATASTORE_DATA_URL"))
	...
	rmcResponseStream.WriteListStructure(pReqUpdateInfo)
